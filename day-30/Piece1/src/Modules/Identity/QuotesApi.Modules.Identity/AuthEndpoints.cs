...
public static class AuthEndpoints
{
    ...
    {
        // REGISTER — a real signup, not the hardcoded dev-seed user. Issues
        // tokens immediately on success (same shape as login) so a new
        // account is usable without a separate login round-trip.
            ...
            IOptions<JwtOptions> jwtOptions) =>
        {
            ...
            var tokens = await IssueTokensAsync(user, context, db, jwtOptions.Value);
    ...
    // Shared by register and login — both end with "issue an access token,
    // write a new refresh token row, set the cookie" and previously
    // duplicated all of it.
    private static async Task<object> IssueTokensAsync(
        User user,
        HttpContext context,
        IdentityDbContext db,
        JwtOptions jwtOptions)
    {
        var accessToken = CreateAccessToken(user, jwtOptions);
        ...
        var expiresInMinutes = (int)jwtOptions.AccessTokenLifetime.TotalMinutes;

        return new { access_token = accessToken, expires_in = expiresInMinutes * 60 };
    }
