...
public static class QuotesEndpoints
{
    ...
    public static IEndpointRouteBuilder MapQuotesEndpoints(this IEndpointRouteBuilder app)
    {
            ...
            CancellationToken cancellationToken) =>
        {
            ...
            // Projected, not the raw entity: Tag has its own back-reference
            // to Quotes, and serializing the entity straight through walks
            // Quote.Tags -> Tag.Quotes -> Quote.Tags forever. Invisible
            // until Tags was actually populated (the .Include fix above),
            // at which point every quote with a tag 500'd.
            return Results.Ok(new QuoteDetailResponse(
                ...
                quote.UserId,
                quote.CreatedAt,
                quote.Tags.Select(t => new TagResponse(t.Id, t.Name)).ToList()));
        });
            ...
            bool? mine) =>
        {
            ...
            int? myQuotesUserId = null;
            if (mine == true)
            {
                if (!TryGetUserId(user, out var userId))
                    return Results.Unauthorized();

                myQuotesUserId = userId;
            }

            var result = await mediator.Send(new GetQuoteFeedQuery(currentPage, currentSize, sortOrder, search, myQuotesUserId));
    ...
    public record QuoteDetailResponse(int Id, string Author, string Text, int UserId, DateTime CreatedAt, List<TagResponse> Tags);
}
