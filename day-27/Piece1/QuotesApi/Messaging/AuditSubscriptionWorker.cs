...
public class AuditSubscriptionWorker : BackgroundService
{
    ...
    {
        // The Azure SDK's own Process activity (subscribed via .AddSource("Azure.*") in
        // Program.cs) already picked up the traceparent this message carries from
        // OutboxRelay's send — Activity.Current here is a child of that send span, so this
        // TraceId matches the one logged by the relay and, through its Link, the original
        // API request.
        ...
        // The DB write that makes this the "worker -> DB" leg of the trace. Runs inside the
        // ServiceBus SDK's own Process activity (args.Message carried the sender's traceparent
        // in its application properties), so EF's instrumentation nests this as a child span
        // under that — visible in App Insights as one trace across send -> process -> SQL.
