...
{
    private static readonly object SqlLogLock = new();
        ...
        services.AddDbContext<QuoteDbContext>(options =>
        {
            ...
            options.LogTo(
                line =>
                {
                    lock (SqlLogLock)
                    {
                        File.AppendAllText(sqlLogPath, line + Environment.NewLine);
                    }
                },
        ...
        this IEndpointRouteBuilder app)
    {
        ...
        // Cached counterpart of the endpoint above: same repository call,
        // wrapped in HybridCache. GetOrCreateAsync gives stampede protection
        // for free — concurrent callers with a cache miss on the same key
        // share one in-flight factory call instead of each hitting the DB.
        ...
        app.MapPost("/cache/quotes/{id:int}/evict", async (int id, HybridCache cache, CancellationToken ct) =>
        {
            await cache.RemoveAsync($"quote:{id}", ct);
            return Results.NoContent();
        })
        .RequireAuthorization();

        app.MapGet("/cache/db-hits", (DbHitCounter hitCounter) => Results.Ok(new { dbHits = hitCounter.Count }))
            .RequireAuthorization();

        app.MapPost("/cache/db-hits/reset", (DbHitCounter hitCounter) =>
        {
            hitCounter.Reset();
            return Results.NoContent();
        })
